...
local window = library:AddWindow("Treee | Private | Killing", {
	main_color = Color3.fromRGB(96, 96, 96),
	min_size = Vector2.new(438, 606),
})
...
local frameToggle = MainTab:AddSwitch("Hide Stat Frames", function(bool)
...
local safePlatform
local megaSwitch = KillingTab:AddSwitch("Egg + Nan Combo Spawn", function(state)
...
local posLockConnection
local safePlatform
local megaSwitch = KillingTab:AddSwitch("Egg + Nan Combo Anti Slam", function(state)
    megaSwitchEnabled = state
    if state then
        local posX, posY, posZ = 100000, -200, 100000
        local character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
...
KillingTab:AddSwitch("Auto Punch Without Animation", function(state)
...
counterBlacklistLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
counterBlacklistLabel.TextSize = 17
...
local cachedREvents = ReplicatedStorage:WaitForChild("rEvents", 5)
local cachedEquipPetEvent = cachedREvents and cachedREvents:WaitForChild("equipPetEvent", 5)
local size2NaNActive = false
local size2NaNThread = nil
local selectedSize2NaNTarget = nil
local function firePetEvent(action, pet)
    if not cachedEquipPetEvent then return end
    pcall(function()
        cachedEquipPetEvent:FireServer(action, pet)
    end)
end
local function unequipAllPets()
    ...
    for _, folder in pairs(petsFolder:GetChildren()) do
        if folder:IsA("Folder") then
            for _, pet in pairs(folder:GetChildren()) do
                if not size2NaNActive then break end
                firePetEvent("unequipPet", pet)
            end
        end
    end
end
local function equipPetsByName(name, maxCount)
    local petsFolder = Player:FindFirstChild("petsFolder")
    ...
    for i = 1, math.min(maxCount or 8, #pets) do
        if not size2NaNActive then break end
        firePetEvent("equipPet", pets[i])
    end
end
local function startSize2NaN()
    if size2NaNActive then return end
    size2NaNActive = true
    size2NaNThread = task.spawn(function()
        while size2NaNActive do
            unequipAllPets()
            task.wait(0.02)
            equipPetsByName("Wild Wizard", 8)
            task.wait(0.02)
            unequipAllPets()
            task.wait(0.02)
            equipPetsByName("Mighty Monster", 8)
            task.wait(0.04)
        end
    end)
end
local function stopSize2NaN()
    size2NaNActive = false
    if size2NaNThread then task.cancel(size2NaNThread) size2NaNThread = nil end
end
local size2NaNDropdown = GodmodeTab:AddDropdown("Select Victim", function(text)
    for _, plr in ipairs(Players:GetPlayers()) do
        local optionText = plr.DisplayName .. " | " .. plr.Name
        if text == optionText then
            selectedSize2NaNTarget = plr
            break
        end
    end
end)
for _, plr in ipairs(Players:GetPlayers()) do
    if plr ~= Player then
        size2NaNDropdown:Add(plr.DisplayName .. " | " .. plr.Name)
    end
end
Players.PlayerAdded:Connect(function(plr)
    if plr ~= Player then
...
Players.PlayerRemoving:Connect(function(plr)
    if selectedSize2NaNTarget == plr then
        selectedSize2NaNTarget = nil
    end
end)
local size2NaNSwitch = GodmodeTab:AddSwitch("Pack Spam", function(bool)
    if bool then
        startSize2NaN()
    else
        stopSize2NaN()
...
local followDropdown = GodmodeTab:AddDropdown("Teleport To Player", function(selectedDisplayName)
...
GodmodeTab:AddSwitch("Auto Ground Slam", function(state)
...
GodmodeTab:AddButton("Stop Following", function()
    following = false
    followTarget = nil
...
local waterLockCFrame = CFrame.new(227.88, -103.76, -788.79)
local waterLockActive = false
local waterLockThread = nil
local waterLockCharConn = nil
local function startWaterLock()
    waterLockActive = true
    local char = Player.Character
    if char then
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if hrp then hrp.CFrame = waterLockCFrame end
    end
    waterLockThread = task.spawn(function()
        while waterLockActive do
            local character = Player.Character
            if character then
                local hrp = character:FindFirstChild("HumanoidRootPart")
                if hrp then
                    hrp.CFrame = waterLockCFrame
    ...
    waterLockCharConn = Player.CharacterAdded:Connect(function(newChar)
        if not waterLockActive then return end
        task.wait(0.05)
        local hrp = newChar:WaitForChild("HumanoidRootPart", 2)
        if hrp then hrp.CFrame = waterLockCFrame end
    end)
end
local function stopWaterLock()
    waterLockActive = false
    if waterLockThread then task.cancel(waterLockThread) waterLockThread = nil end
    if waterLockCharConn then waterLockCharConn:Disconnect() waterLockCharConn = nil end
end
local waterSwitch = GodmodeTab:AddSwitch("Break GodMode", function(bool)
    if bool then startWaterLock() else stopWaterLock() end
end)
waterSwitch:Set(false)