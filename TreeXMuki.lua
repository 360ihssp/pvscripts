...
Tabs.Main:AddButton("Anti Lag", function()
...
Tabs.Main:AddButton("Infinite Yield", function()
...
local EvilKarmaFrame = ReplicatedStorage:WaitForChild("evilKarmaFrame")
local GoodKarmaFrame = ReplicatedStorage:WaitForChild("goodKarmaFrame")
local StrengthFrame = ReplicatedStorage:WaitForChild("strengthFrame")

local function SetKarmaFramesEnabled(isEnabled)
    EvilKarmaFrame.Visible = isEnabled
    GoodKarmaFrame.Visible = isEnabled
	StrengthFrame.Visible = isEnabled
end

Tabs.Main:AddSwitch("Disable Frames", function(isDisabled)
    SetKarmaFramesEnabled(not isDisabled)
end):Set(true)
...
MiscellaneousLabel.TextSize = 50
...
Tabs.Killing:AddSwitch("Disable eating Eggs", function(bool)
...
local fileName = "TreeBlacklist_" .. LocalPlayer.Name .. ".txt"
...
local function rebuildCache()
	table_clear(cachedBlacklist)
	for i = 1, #blacklistWords do
		cachedBlacklist[i] = blacklistWords[i]:lower()
	end
	blacklistDirty = false
end

local function updateBlacklistLabel(label)
	label.Text = (#blacklistWords == 0)
		and "Clan Blacklist: None"
		or ("Clan Blacklist: " .. table.concat(blacklistWords, ","))
end

local function parseList(text)
	table_clear(blacklistWords)
	if text and text ~= "" then
		for w in string_gmatch(text, "[^,]+") do
			local t = trim(w):lower()
			if t ~= "" then
				table_insert(blacklistWords, t)
			end
		end
	end
	blacklistDirty = true
...
local function nameMatchesAny(player)
	...
	for i = 1, #cachedBlacklist do
		local w = cachedBlacklist[i]
		if w ~= "" and string_find(dn, w, 1, true) then
			return true
		end
	end
	return false
...
local function getHands(char)
	while char and not (char:FindFirstChild("RightHand") or char:FindFirstChild("Right Arm")) do
		task_wait()
	end
	return
	...
	if not punch then
		task.defer(function()
			for _ = 1, 40 do
				...
				local p = backpack:FindFirstChild("Punch")
				if p then
					p.Parent = char
					break
				end
...
updateBlacklistLabel(blacklistLabel)

local function applyUIUpdate()
	updateBlacklistLabel(blacklistLabel)
	refreshActive()
...
Tabs.Killing:AddTextBox("Add to Blacklist", function(txt)
	parseList(table.concat(blacklistWords, ",") .. "," .. txt)
	saveList()
	applyUIUpdate()
end, { placeholder = "Ej: MVX, Sigma, Juan" })

Tabs.Killing:AddTextBox("Remove from Blacklist", function(txt)
	local toRemove = {}
	for w in string_gmatch(txt, "[^,]+") do
		local t = trim(w):lower()
		if t ~= "" then
			table_insert(toRemove, t)
		end
	end
	for i = 1, #toRemove do
		local word = toRemove[i]
		for j = #blacklistWords, 1, -1 do
			if blacklistWords[j] == word then
				table.remove(blacklistWords, j)
			end
		end
	end
	saveList()
...
task.spawn(function()
	while true do
		...
		if not muscleEvent then continue end
...
Players.PlayerAdded:Connect(function(plr)
	plr:GetPropertyChangedSignal("DisplayName"):Connect(refreshActive)
end)
...
local teleportLocations = {
	...
	{name = "Water", pos = CFrame.new(-8, 0, -1000)}
}
...
local AutoGymTeleport = false

local AutoGymTPSwitch = Tabs.GodMode:AddSwitch("Auto gym TP", function(bool)
	AutoGymTeleport = bool
	if bool then
		task.spawn(function()
			local index = 1
			while AutoGymTeleport do
				local character = PlayerData.Player.Character
				local hrp = character and character:FindFirstChild("HumanoidRootPart")
				if hrp then
					hrp.CFrame = Locations[index].pos
					index = index + 1
...
AutoGymTPSwitch:Set(false)

local isAutoTpEnabled = false

local GodModeCounterSwitch = Tabs.GodMode:AddSwitch("Auto tp To Water", function(enabled)
    isAutoTpEnabled = enabled
end)
...
spawn(function()
    while true do
        if isAutoTpEnabled then
            ...
            local humanoidRootPart = (player.Character or player.CharacterAdded:Wait()):WaitForChild("HumanoidRootPart", 0.1)
            if humanoidRootPart then
                humanoidRootPart.CFrame = CFrame.new(-8, 0, -1000)
            end
        end
        wait()
    end
...
local SelectTargetDropdown = Tabs.GodMode:AddDropdown("Select Target", function(txt)
    for _, p in ipairs(Players:GetPlayers()) do
...
for _, p in ipairs(Players:GetPlayers()) do
    if p ~= Player then
        SelectTargetDropdown:Add(p.DisplayName .. " | " .. p.Name)
    end
end
...
Tabs.GodMode:AddSwitch("Kill Teleport", function(v)
